    ...
    public class TasksController : ControllerBase
    {
        ...
        public async Task<IActionResult> DeleteTask(int idTask)
        {
            try
            {
                ...
                var existingTask = await _taskRepo.GetByIdAsync(idTask);
                if (existingTask == null)
                {
                    return NotFound(); // Devuelve un 404 si la tarea no existe
                }

                // Elimina la tarea utilizando el repositorio
                await _taskRepo.DeleteAsync(idTask);

                return NoContent(); // Devuelve un 204 No Content si la tarea se eliminó con éxito
            }
            catch (Exception ex)
            {
                // Manejo de errores, registra el error y devuelve un resultado apropiado
